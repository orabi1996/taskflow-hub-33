...
test("projects routing & UI invariants", () => {
  ...
  assert(
    detail.includes('data-testid="project-skeleton"'),
    "صفحة التفاصيل تعرض Skeleton أثناء التحميل",
  );
  ...
  assert(/error\s*\|\|\s*!project/.test(detail), "Error UI يظهر عند وجود خطأ أو غياب البيانات");
  assert(
  ...
  assert(
    /onClick=\{\(\)\s*=>\s*load\("refresh"\)\}/.test(dashboard),
    "زر تحديث متصل بـ load(refresh)",
  );
  ...
  assert(
    list.includes("healthFilter") && list.includes("statusFilter"),
    "فلاتر الحالة الصحية والنشاط",
  );
});
